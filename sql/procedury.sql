...
BEGIN
    ...
    DECLARE
        @RESULT INT
    EXEC
        @RESULT = PROC_3_5_VYPOCET_HODIN_OBDOBI
                  @P_OBDOBIOD,
                  @P_OBDOBIDO,
                  @P_IDOSOBA,
...
BEGIN
    ...
    DECLARE
        @RESULT INT
    EXEC
        @RESULT = PROC_3_6_VYPOCET_HODIN_CELKEM
                  @P_IDOSOBA,
                  @PO_HODINCELKEM OUTPUT
...
BEGIN
    ...
    DECLARE
        @RESULT INT
    EXEC
        @RESULT = PROC_3_7_VYPOCET_PLATU
                  @P_OBDOBIOD,
                  @P_OBDOBIDO,
                  @P_IDOSOBA,
                  @PO_MESICNIPLAT OUTPUT

    SELECT @PO_MESICNIPLAT AS 'MESICNI PLAT',
           @RESULT         AS RESULT
END
...
BEGIN
    ...
    DECLARE
        @RESULT INT
    EXEC
        @RESULT = PROC_7_1_NAPLANOVANY_NA_PROBEHLY
                  @P_IDKROUZEK,
                  @P_DATUM,
                  @P_ZRUSEN,
                  @P_POCETZAKU
...
BEGIN
    ...
    DECLARE
        @RESULT INT
    EXEC
        @RESULT = PROC_9_4_ZVYSENI_MZDY_LEKTOROVI
                  @P_IDLEKTOR,
                  @P_PLATNOSTOD
END
...
BEGIN
    ...
    DECLARE
        @RESULT INT
    EXEC
        @RESULT = PROC_6_5_ZOBRAZENI_KROUZKU
                  @P_DATUMOD,
                  @P_DATUMDO,
                  @P_AKTUALNIDATUM
END

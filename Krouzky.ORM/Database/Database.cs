...
namespace Krouzky.ORM.Database {
    ...
    public class Database {
        ...
        /// </summary>
        public bool Connect() {
            if (this.Connection == null || this.Connection.State == ConnectionState.Open) return false;
...
//            return this.Connect(ConfigurationManager.ConnectionStrings["ConnectionStringOracle"].ConnectionString);
        ...
        public int ExecuteNonQuery(SqlCommand command) {
            int rowNumber = 0;
            try {
                if (command != null) rowNumber = command.ExecuteNonQuery();
            }
            catch (Exception e) {
                throw e;
            }
        ...
        public SqlCommand CreateCommand(string strCommand) {
            ...
            if (this.SqlTransaction != null) command.Transaction = this.SqlTransaction;
